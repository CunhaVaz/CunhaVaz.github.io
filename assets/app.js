(function () {
  ...
  async function answerQuestion(q) {
    ...
    const T = isEN
      ? { llm: 'Mode: LLM (backend)', local: 'Mode: local search', intro1: 'Answer based on the CV:', introN: n => 'Summary from ' + n + ' sections of the CV:', close: '<br/><br/><em>For direct contact: francisco.vaz@ubi.pt</em>', notFound: [
          ...
          "",
          "For direct contact: <strong>francisco.vaz@ubi.pt</strong> · <strong>+351 926 771 211</strong>"
        ].join('<br/>') }
      : { llm: 'Modo: LLM (backend)', local: 'Modo: pesquisa local', intro1: 'Resposta com base no CV:', introN: n => 'Resumo a partir de ' + n + ' secções do CV:', close: '<br/><br/><em>Para conversa direta: francisco.vaz@ubi.pt</em>', notFound: [
          ...
          "",
          "Para contacto direto: <strong>francisco.vaz@ubi.pt</strong> · <strong>+351 926 771 211</strong>"
        ].join('<br/>') };
  ...
  function tokenize(s) {
    return s.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter(t => t.length > 2);
  }
